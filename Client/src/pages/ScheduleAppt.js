...
<option selected>Doctors Available</option>
  {doctorName && doctorName.map ((doctor,index) =>  <option key={index} value={index}>{doctor.firstName + " " + doctor.lastName}</option>)}
</select>
<label htmlFor="">Input Doctor Name</label>
    <input type="text" name='doctorName' value={formData.doctorName} onChange={HandleChange}/>
<label for="party" className="mt-3">Enter a date and time for your Schedule:</label>
<input id="party" type="datetime-local" name="date" value={formData.date} onChange={HandleChange} className="mb-4"/>
          <textarea
            name="Concerns"
            id=""
            cols="30"
